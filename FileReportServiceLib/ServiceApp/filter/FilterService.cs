...
namespace FileReporterApp.ServiceApp.filter
{
    public class FilterService : IFilterService<FileInfo>
    {
        ...
        {
            return timeEnum == TimeEnum.BEFORE ? filterObj.CreationTime.CompareTo(date) < 0 : filterObj.CreationTime.CompareTo(date) >= 0;
        }
        public bool FilterByModifiedDate(FileInfo filterObj, DateTime date, TimeEnum timeEnum)
        {
            return timeEnum == TimeEnum.BEFORE ? filterObj.LastAccessTime.CompareTo(date) < 0 : filterObj.LastAccessTime.CompareTo(date) >= 0;
        }
