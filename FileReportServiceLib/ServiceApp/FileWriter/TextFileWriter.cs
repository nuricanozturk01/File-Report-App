namespace FileReporterApp.ServiceApp.FileWriter
{
    internal class TextFileWriter : IFileWrite
    {
        ...
        private void WriteTextFile(List<FileInfo> scannedMergedList, string targetPath) => File.WriteAllLines(targetPath, scannedMergedList.AsParallel().Select(f => GetFormattedString(f)));
